...
class MVSDataset(Dataset):
    ...
    def read_depth(self, scan, scale, filename):
        # read pfm depth file
        depth_image = np.array(read_pfm(filename)[0], dtype=np.float32)
        depth = depth_image * self.scale_factors[scan] * scale
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            if i == 0:  # reference view
                ...
                depth = self.read_depth(scan, scale, depth_filename)
                h, w = depth.shape
                depth_ms = {
                ...
                mask_ms = {
                    "stage1": np.array((depth_ms["stage1"] >= depth_min) & (depth_ms["stage1"] <= depth_max),
                                       dtype=np.float32),
                    "stage2": np.array((depth_ms["stage2"] >= depth_min) & (depth_ms["stage2"] <= depth_max),
                                       dtype=np.float32),
                    "stage3": np.array((depth_ms["stage3"] >= depth_min) & (depth_ms["stage3"] <= depth_max),
                                       dtype=np.float32),
                    "stage4": np.array((depth_ms["stage4"] >= depth_min) & (depth_ms["stage4"] <= depth_max),
                                       dtype=np.float32),
                }
