...
class MVSDataset(Dataset):
    def __init__(self, datapath, listfile, mode, nviews, ndepths=192, **kwargs):
        ...
        self.interval_scale = 1.06 / (float(ndepths) / 192.0)
        self.kwargs = kwargs
    ...
    def prepare_img(self, hr_img):
        ...
        if not self.use_raw_train:
            ...
            hr_img_crop = hr_img[h // 2 - 1024 // 2:h // 2 + 1024 // 2,
                          w // 2 - 1280 // 2:w // 2 + 1280 // 2]  # 1024, 1280, c
        # #downsample
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            if not self.use_raw_train:
                ...
                img_filename = os.path.join(self.datapath,
                                            'Rectified2/{}/rect_{:0>3}_{}_r5000.png'.format(scan, vid + 1,
                                                                                               light_idx))
            ...
            intrinsics, extrinsics, depth_min_, depth_interval = self.read_cam_file(proj_mat_filename)
            ...
            if i == 0:  # reference view
                ...
                # get depth values
                depth_max_ = depth_interval * self.ndepths + depth_min_
        ...
        # sample_depth_point = np.arange(0, self.ndepths, depth_interval, dtype=np.float32)
        # sample_depth_point = np.linspace(0, self.ndepths-1, num=self.ndepths)
        return {"imgs": imgs,
