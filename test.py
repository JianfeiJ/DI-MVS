...
parser.add_argument('--testpath', default='D:\Data\DTU\DTU_Dataset\dtu\dtu_test_mvsnet', help='testing data dir for some scenes')
...
parser.add_argument('--loadckpt', default='checkpoints/dtu_train.ckpt', help='load a specific checkpoint')
parser.add_argument('--outdir', default='E:\DI-MVSNet-result\DTU\\baseline_pv_LGFI\\test', help='output dir')
...
parser.add_argument('--cost_reg', default='small', choices=['small', 'regular', '2d', '3d'])
...
# filter
parser.add_argument('--conf', type=float, nargs='+', default=[0.5, 0.5, 0.5, 0.5], help='prob confidence')
...
def save_scene_depth(testlist):
    ...
    if args.data_type == 'dtu':
        test_dataset = MVSDataset(args.testpath, testlist, "test", args.num_view, args.numdepth, max_h=args.max_h,
                                  max_w=args.max_w, fix_res=args.fix_res)
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            for filename, cam, img, depth_est, conf_stage0, conf_stage1, conf_stage2, conf_stage3, depth_max_, depth_min_ in zip(
                    filenames, cams, imgs, outputs["depth"][-1],
                    outputs["photometric_confidences"][0], outputs["photometric_confidences"][1],
                    outputs["photometric_confidences"][2], outputs["photometric_confidences"][3],
                    depth_max, depth_min):
                img = img[0]  # ref view
                _, h, w = img.shape
                cam = cam[0]  # ref cam
                ...
                # print(depth_est.min())
                #
                confidence_filename0 = os.path.join(args.outdir, filename.format('confidence', '.pfm'))
                ...
                os.makedirs(confidence_filename0.rsplit('/', 1)[0], exist_ok=True)
                ...
                conf_stage0 = cv2.resize(conf_stage0, (w, h))
                conf_stage1 = cv2.resize(conf_stage1, (w, h))
                conf_stage2 = cv2.resize(conf_stage2, (w, h))
                conf_stage3 = cv2.resize(conf_stage3, (w, h))


                # multiple (m)
                # conf_final = conf_stage1 * (conf_stage2 + conf_stage3 + conf_stage4)

                # conf_final = conf_stage1

                save_pfm(confidence_filename0, conf_stage0)
                save_pfm(confidence_filename1, conf_stage1)
                save_pfm(confidence_filename2, conf_stage2)
                save_pfm(confidence_filename3, conf_stage3)




                # mi = np.min(conf_final[depth_est > 0])
                # ma = np.max(conf_final)
                # depth = (conf_final - mi) / (ma - mi + 1e-8)
                # depth = (255 * depth).astype(np.uint8)
                # depth_img = cv2.applyColorMap(depth, cv2.COLORMAP_VIRIDIS)
                # cv2.imwrite(os.path.join(args.outdir, filename.format('confidence', '_final.png')), depth_img)



                cv2.imwrite(os.path.join(args.outdir, filename.format('confidence', '.png')),
                            visualize_depth(conf_stage0))
                cv2.imwrite(os.path.join(args.outdir, filename.format('confidence', '_1.png')),
                            visualize_depth(conf_stage1))
                cv2.imwrite(os.path.join(args.outdir, filename.format('confidence', '_2.png')),
                            visualize_depth(conf_stage2))
                cv2.imwrite(os.path.join(args.outdir, filename.format('confidence', '_3.png')),
                            visualize_depth(conf_stage3))
...
def check_geometric_consistency_tank(depth_ref, intrinsics_ref, extrinsics_ref, depth_src, intrinsics_src,
                                     extrinsics_src, dh_pixel_dist_num):
    width, height = depth_ref.shape[1], depth_ref.shape[0]
    x_ref, y_ref = np.meshgrid(np.arange(0, width), np.arange(0, height))
...
def filter_depth_tank(scan, pair_folder, scan_folder, out_folder, plyfilename):
    ...
    for ref_view, src_views in pair_data:
        ...
        photo_mask = np.logical_and(np.logical_and(confidence > args.conf[2], confidence1 > args.conf[1]),
                                    confidence2 > args.conf[2], confidence3 > args.conf[3])
...
def filter_depth(pair_folder, scan_folder, out_folder, plyfilename):
    ...
    for ref_view, src_views in pair_data:
        ...
        photo_mask = np.logical_and(np.logical_and(confidence > args.conf[0], confidence1 > args.conf[1]),
                                    confidence2 > args.conf[2], confidence3 > args.conf[3])


        # photo_mask = confidence > args.conf[0]
...
if __name__ == '__main__':
    ...
    for scan in testlist:
        ...
        if not os.path.exists(plypath):
            os.makedirs(plypath)
        if args.data_type == 'tank':
            filter_depth_tank(scan, pair_folder, scan_folder, out_folder,
                              os.path.join(args.outdir, 'plyfilter/{}.ply'.format(scan)))
            ...
            filter_depth(pair_folder, scan_folder, out_folder,
                         os.path.join(args.outdir, 'plyfilter/mvsnet{:0>3}_l3.ply'.format(scan_id)))
        else:
            print("wrong data_type")
