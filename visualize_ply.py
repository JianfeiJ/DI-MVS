...
if __name__ == "__main__":
    ...
    parser.add_argument('--scan', type=str, default='Temple',
                        help='the scan to visualize')
    ...
    if args.dataset == 'dtu':
        path = os.path.join(args.logdir, 'mvsnet{:0>3}_l3.ply'.format(args.scan))
    ...
    if args.use_viewpoint:
        ...
        param = o3d.io.read_pinhole_camera_parameters("viewpoints/viewpoint_scan12.json")
        ctr.convert_from_pinhole_camera_parameters(param)
        vis.run()
    elif args.save_viewpoint:
        vis.run()
        param = ctr.convert_to_pinhole_camera_parameters()
        o3d.io.write_pinhole_camera_parameters("viewpoints/viewpoint_scan12.json", param)
    else:
        vis.run()
